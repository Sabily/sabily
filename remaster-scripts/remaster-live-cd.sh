...
function unpack_initrd()
{
	...
	elif [ -e "$ISO_REMASTER_DIR/install/initrd.lz" ]; then
		INITRD_FILE="$ISO_REMASTER_DIR/install/initrd.lz"
	else
		failure "Can't find initrd.lz file"
	fi

	cat "$INITRD_FILE" | lzma -dc -S .lz | cpio -i
	RESULT=$?
	if [ $RESULT -ne 0 ]; then
...
function pack_initrd()
{
	...
	if [ -e "$ISO_REMASTER_DIR/casper" ]; then
		...
		failure "Can't find where to copy the initrd.lz file"
	fi
